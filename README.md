...
### v0.9.27
- 🎨 **加载按钮优化**：改为简洁的向上箭头图标
- ⚡ **滚动停止优化**：连续 2 次无新内容就停止（原为 3 次）
- 🐛 **修复语法错误**：修复 `oldCount` 变量重复声明问题
- 📝 **文档更新**：修正版本号和 GitHub 链接

### v0.9.26
- 🔄 **加载全部功能**：新增"加载全部"按钮，自动滚动到顶部加载历史消息
- 🎯 **智能停止**：连续 3 次无新内容自动停止
- 💾 **状态显示**：实时显示加载进度和轮次

### v0.9.25
- 🔍 **搜索功能增强**：改进搜索算法，提升匹配准确度
- 🎯 **精确跳转**：优化跳转逻辑，定位更准确
...
### v0.9.23
- 🐛 **修复跳转**：改进 window.find() 滚动逻辑
- 🔄 **自动重搜**：滚动加载时自动更新搜索结果

### v0.9.16
- ✨ **高亮动画**: 跳转后目标句子会闪烁高亮 3 秒
  - 🌟 黄色渐变背景
  - 💫 脉冲动画效果（闪烁 3 次）
  - 🎯 自动移除高亮（3 秒后）
  - 📱 平滑滚动到视口中央
